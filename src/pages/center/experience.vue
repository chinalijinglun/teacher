...
import subjectTable from '../applyProcess/comps/subjectTable';
export default {
  ...
  created() {
    const editForm = this.$getSession('experienceEdit')
    const deleteHistory = this.$getSession('deleteHistory')
    if(deleteHistory) {
      this.deleteHistory = JSON.parse(deleteHistory);
    }
    if(editForm) {
  ...
  watch: {
    form: {
      handler(v) {
        this.$setSession('experienceEdit', JSON.stringify(v))
      },
      deep: true
    }
  },
  methods: {
    getTeacherInfo() {
      return Promise.all([
        this.getTeacherBasic(),
        this.getSubject(1), // other subjects
        this.getSubject(2) // cur subjects
      ]).then(([basicInfo, otherSubjects, curSubjects]) => {
        this.form = {
          ...basicInfo,
          subject: '1', // 样式统一
          qualified_objects: '1', // 样式统一
        };
        this.deleteHistoryLs = [...curSubjects, ...otherSubjects]
        this.$setSession('deleteHistory', JSON.stringify(this.deleteHistoryLs))
        this.$refs.subjectTable2.init(curSubjects)
        this.$refs.subjectTable1.init(otherSubjects)
      })
    },
    getTeacherBasic() {
      return teacherGetByTeacherid(this.userId).then(resp => {
        ...
        return {
          ...
          teacher_age,
          resume_url,
          seniority_url: JSON.parse(seniority_url || '[]'),
          teaching_history
        }
      });
    },
    getSubject(type) {
      return teacherSubject({
        ...
        this.countryLs = data;
        if(this.form.cur_country) {
          this.form.cur_country = +this.form.cur_country
          this.getProvinceLs(this.form.cur_country).then(_=>{
            if(this.form.cur_province) {
              this.form.cur_province = +this.form.cur_province;
            }
          })
        }
			})
      ...
      this.$refs.basicForm.validate().then(() => {
        const deleteHistoryPromises = this.deleteHistoryLs.map(item => this.deleteHistory(item.id));
    ...
    basicUpdated() {
			const form1 = JSON.parse(this.$getSession('basicInfoEdit') || '{}');
      const {
        cur_school,
				cur_country,
        cur_province,
